...
public class ComponentIndexTest {
  ...
  @Rule
  public UserSessionRule userSession = UserSessionRule.standalone().login("john").setUserId(TEST_USER_ID).setUserGroups(TEST_USER_GROUP);
  ...
  private ComponentDto indexProject(String key, String name) {
    return index(
      ComponentTesting.newProjectDto("UUID_" + key)
        .setKey(key)
        .setName(name));
  }

  private ComponentDto newProject(String key, String name) {
    return ComponentTesting.newProjectDto("UUID_" + key)
      .setKey(key)
      .setName(name);
  }
